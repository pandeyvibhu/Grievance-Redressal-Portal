...
def multilabelbinarizer_classification(norm_corpus, category, norm_query_docs, category_test, validation=False):
    ...
    save_models = open("similarity.pickle", "wb")
...
def retrain_multilabelbinarizer_model(query_post, category_unused):
    t01 = time()
...
def classify(problem):
    query_post= make_query_post(problem)
    t04 = time()
    pickled_data_loc = "/home/vishesh/btp/main/similarity_p3.pickle"
    ...
    for item, labels in zip(X_test, all_labels):
        ...
        print('{0} => {1}'.format('Category', ', '.join(labels)))
        categories_decided += ' '
        categories_decided += ' '.join(labels)

    print("Classification completed in ", round(time()-t04, 3), "s")
    return categories_decided[1:]

def make_query_post(problem):
